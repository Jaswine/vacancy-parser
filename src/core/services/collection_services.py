...
class CollectionService:
    ...
    async def find_collections_paginated_by_account_id(self, account_id: UUID, 
        ...
        """
        return await self.collection_repository.find_collections_paginated_by_account_id(account_id, 
                                                                                             page, page_size)
    ...
    async def find_collection_specific_data_by_id(self, collection_id: UUID) -> CollectionFindOneSchema:
        """
        Get a collection by its ID
        """
        return await self.collection_repository.find_collection_specific_data_by_id(collection_id)
    
    async def get_total_collections_count(self, account_id: UUID) -> int:
        """
        Get total number of collections for a given account ID
        """
        return await self.collection_repository.count_collections_by_account_id(account_id)
