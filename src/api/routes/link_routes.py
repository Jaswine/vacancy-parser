...
async def get_all_links(
    ...
    try:
        ...
        links: List[
            LinkFindAllSchema
        ] = await service.find_links_paginated_by_collection_id(
            collection_id, page, page_size
        )

        # Build and return response
        return LinkFindAllPaginationSchema(
            page=page, page_size=page_size, total=links_count, items=links
        )
...
async def create_link(
    ...
    link_service: LinkService = LinkService(
        db, link_repository, collection_link_repository
    )
...
async def remove_link(
    ...
    collection_link_service: CollectionLinkService = CollectionLinkService(
        collection_link_repository
    )
    ...
    try:
        ...
        # Get link by id
        link = await link_service.get_link_by_id(link_id)
        if not link:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
